...
PNG createSpotlight(PNG image, int centerX, int centerY) {
  ...
  for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
      HSLAPixel & pixel = image.getPixel(x, y);     //get pixel at each location

      //calculate distance
      int dx = centerX - x;  // finds x-difference between current and centerY
      int dy = centerY - y;  // finds y-difference between current and centerX
      int dist = std::sqrt( dx*dx + dy * dy);  // calculated Euclidean distance of pixel from center

      // apply appropriate luminance change
      if (dist > 160) // pixels over 160 px away have a uniform change
      {
        pixel.l = pixel.l * 0.2;  // decrease luminance by 80%
      }
      else
      {
        double multiplier = 1 - (.005 * dist); // calculates factor to multiply luminance by
        pixel.l = pixel.l * multiplier; // modifies luminance
      }
    }
  }
  return image;
...
PNG watermark(PNG firstImage, PNG secondImage) {
  for (unsigned x = 0; x < firstImage.width() && x < secondImage.width(); x++) {
    for (unsigned y = 0; y < firstImage.height() && y < secondImage.height(); y++) {
        ...
        HSLAPixel & firstpixel = firstImage.getPixel(x, y);     //get pixel at each location
        HSLAPixel & secondpixel = secondImage.getPixel(x, y);     //get pixel at each location

        if(secondpixel.l == 1 && firstpixel.l + 0.2 <= 1)
            firstpixel.l += 0.2;

        else if(secondpixel.l == 1 && firstpixel.l + 0.2 > 1)
            firstpixel.l = 1;
    }
  }
  return firstImage;
