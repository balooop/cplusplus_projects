                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    if(singleQueue.size() == 0 && mergeQueue.size() == 0)
        return NULL;
    //gets front of each queue
    HuffmanTree::TreeNode * front;

    //if only one of the lists is populated
    ...
    //both lists have at least one item
    else{
        //gets frequencies
        int singleFreq = singleQueue.front()->freq.getFrequency();
        int mergeFreq = mergeQueue.front()->freq.getFrequency();

        //case where mergequeue has the smaller item
        ...
        //case where singlequeue has the smaller item
        else
        {
            TreeNode * singleFront = singleQueue.front();
            singleQueue.pop();
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    int totalSize = singleQueue.size() + mergeQueue.size();
    while(totalSize != 1)
    {
        //gets two smallest nodes
        ...
        int combinedFreq = temp1->freq.getFrequency() + temp2->freq.getFrequency();
        TreeNode * internal = new TreeNode(combinedFreq);
        
        //assigns internal node's kids
        internal->left = temp1;
        ...
        totalSize = singleQueue.size() + mergeQueue.size();
    }
    ...
    if(mergeQueue.size() == 0 && singleQueue.size() == 1)
    {
...
void HuffmanTree::decode(stringstream& ss, BinaryFileReader& bfile)
{
    ...
    while (bfile.hasBits()) {
         ...
         */
        int currentBit = bfile.getNextBit();
        //if 0, go left
        if(currentBit == 0)
        ...
        //if it's a leaf, it has no kids
        if(current->left == NULL && current->right == NULL)
        {
            //so we get the char byte, and put itt in the stringstream
            char leaf = current->freq.getCharacter();
            ss << leaf;
            //and then we start going back from the root
            current = root_;
        }
    }
}
...
void HuffmanTree::writeTree(TreeNode* current, BinaryFileWriter& bfile)
{
    ...
    if(current->left == NULL && current->right == NULL)
    {
        //writes 1 bit and character byte
        bfile.writeBit(1);
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    {
        //gets frequency
        Frequency newFreq = Frequency((char)bfile.getNextByte(), 0);
        //creates new leaf node
        TreeNode * newLeaf = new TreeNode(newFreq);
        return newLeaf;
    }
    if(currentBit == 0)
    {
