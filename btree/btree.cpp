...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
    ...
    //data pair at the first larger index of what we want to find
    DataPair idx = subroot->elements[first_larger_idx];
    BTreeNode * child = subroot->children[first_larger_idx];

    //if first larger index is within the size of the elements array
    if(first_larger_idx < subroot->elements.size())
    {
        //if the key exists in the elements array, we return the value
        if(idx.key == key)
        {
            return V(idx.value);
        }
    }
    //if its a leaf, we return a default V() base case
    if(subroot->is_leaf)
    {
        return V();
    }else{
...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    //inserts child iterator at the new spot in parent's children array 
    parent->children.insert(child_itr, new_right);
    //inserts mid element into parent node's element array
    parent->elements.insert(elem_itr, *mid_elem_itr);
    
    //replaces right child's elements with the right half of the split child's elements 
    new_right->elements.assign(mid_elem_itr+1, child->elements.end());
    //replaces left child's elements with the left half of the split child elements
    new_left->elements.assign(child->elements.begin(), mid_elem_itr);
    //assigns children's children elements if it isn't a leaf
    if(!child->is_leaf){
...
/*
    parent->elements.insert(elem_itr, *mid_elem_itr);
    //inserts child item into the right child spot
    parent->children.insert(child_itr, new_right);

    //assigns right element and right child (if child isn't a leaf)
    new_right->elements.assign(mid_elem_itr + 1, child->elements.end());
    new_left->elements.assign(child->elements.begin(), mid_elem_itr);
    if(!child->is_leaf)
    {
        new_right->children.assign(mid_child_itr, child->elements.end());
        new_left->children.assign(child->elements.begin(), mid_child_itr);
    }
*/
}   
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    /* TODO Your code goes here! */
    //
    auto ins_idx = first_larger_idx + subroot->elements.begin();    
    //leaf case
    if(subroot->is_leaf)
    {
        //insert into subroot
        subroot->elements.insert(ins_idx, pair);
    }else{
        //recursive call to insert at correct spot
        insert(subroot->children[first_larger_idx], pair);
        //checks if split is needed
