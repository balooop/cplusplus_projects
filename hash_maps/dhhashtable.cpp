...
void DHHashTable<K, V>::insert(K const& key, V const& value)
{
    ...
    //increments elems
    ++elems;
    //calculates hash index and step size
    unsigned hash_idx = hashes::hash(key, size);
    unsigned step = hashes::secondary_hash(key, size);

    //creates pair to be inserted
    std::pair<K,V> * insert_pair = new std::pair<K,V>(key, value);
   
    //keeps stepping to find an open spot
    while(table[hash_idx] != NULL)  hash_idx = (hash_idx+step) % this->size;
    //inserts into right spot
    table[hash_idx] = insert_pair;
    should_probe[hash_idx] = true;

    //checks if we should resize
...
int DHHashTable<K, V>::findIndex(const K& key) const
{
     ...
     * @todo Implement this function
     */
    //gets step size and hash index
    unsigned hash_idx = hashes::hash(key, size);
    unsigned start_idx = hash_idx;
    unsigned step = hashes::secondary_hash(key, size);
    
    //loops while elements exist at index
    while(should_probe[hash_idx])
    {    
        //if there's an element and the element has the key we're looking for, we are done
        if(table[hash_idx] != NULL && table[hash_idx]->first == key)    return hash_idx;
        //step increments index
        hash_idx = (hash_idx + step) % this->size;
        //dont want to infinitely loop
