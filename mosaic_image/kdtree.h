...
class KDTree
{
  ...
  private:
    ...
    void travelDown(Point<Dim> &closest, const Point<Dim> &query, KDTreeNode* treeNode, int dim) const;
    //distance calculator helper for shouldReplace
    double _calculateDistance(const Point<Dim> & a, const Point<Dim> &b) const;
    //quickselect helper for constructor
    Point<Dim>& _quickselect(unsigned l, unsigned r, unsigned d, unsigned k);
    //partition helper for qucikselect
    unsigned _partition(unsigned left, unsigned right, unsigned d, unsigned k);
    //recursive helper function for ctor
    KDTreeNode *  _construct(unsigned l, unsigned r, unsigned d);
    //rule of three destroy helper
    void _destroy(KDTreeNode *& root);
    //rule of three copy helper
    void _copy(KDTreeNode *& curr, KDTreeNode * other);
